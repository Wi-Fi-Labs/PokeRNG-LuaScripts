...
elseif gameLang == 0x4A then
 ...
 pokemonStatsScreen2Addr = 0x020032A0
 pokemonBattleStatsScreenAddr = 0x020119C4
 currBoxNumberPointerAddr = 0x03004F60
 boxSelectedSlotNumberAddr = 0x02039821
end
...
function getInstructionsInput()
 ...
 if mode[index] == "100% Catch" and showInstructionsText then
  ...
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 72, "4) Advance a single frame (CTRL+N) while holding SELECT")
 ...
 elseif mode[index] == "Initial Seed Bot" and showInstructionsText then
  ...
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 36, "2) Reboot the core")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 54, "3) Advance a single frame (F) + holding 1")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 72, "4) Advance a single frame (F)")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 90, "5) Advance a single frame (F) + holding 1")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 108, "6) Advance a single frame (F)")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 126, "7) Advance a single frame (F) + holding 1")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 144, "8) Advance a single frame (F)")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 162, "9) Advance a single frame (F) + holding SELECT")
  gui.text(emuWindow.leftPadding + 1, emuWindow.topPadding + 180, "10) Unpause the game")
...
function checkInitialSeedGeneration(initial, current)
...
function getCleanseTagEffectMod(finalRate)
 local partyLeadMonHeldItem = read8Bit(wildEncounterDataAddr + 0xA)
...
function getAbilityEffectMod(finalRate)
 local partyLeadMonAbilityEffectType = read8Bit(wildEncounterDataAddr + 0x9)
...
function showDayCareInfo()
 ...
 if isEggReady then
  ...
  gui.text(emuWindow.leftPadding + 388, emuWindow.topPadding + 144, string.format("Egg lower PID: %04X", eggLowPid))
...
function advanceToDelay(delay, state)
 state = state or nil
...
function initSeedBotLoop()
 ...
 while delay1 ~= delay5 and not initSeedFound do
  while delay2 ~= delay4 and not initSeedFound do
   while delay3 ~= delay6 and not initSeedFound do
