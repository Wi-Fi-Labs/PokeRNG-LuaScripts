...
function checkInitialSeedGeneration(initial, battle, current)
 ...
 elseif current == read32Bit(timerAddr) then
  paintingSeed = read32Bit(timerAddr)
  tempInit = paintingSeed
  adjustAdvances = currAdvances - calcAdvancesJump(paintingSeed)
 elseif initial == 0 and battle ~= battleVideoSeed2 and battleVideoSeed2 ~= 0 then
  tempInit = 0
  adjustAdvances = 0
 end
end
...
function showRngInfo(initial, current, battle, advances)
 gui.text(emuWindow.leftPadding, emuWindow.bottomPadding - 72, string.format("Initial Seed: %04X", initial))
 gui.text(emuWindow.leftPadding + 1, emuWindow.bottomPadding - 54, string.format("Painting Timer: %08X", read32Bit(timerAddr)))
...
while warning == "" do
 ...
 if paintingSeed ~= 0 then
  initSeed = paintingSeed
 end
