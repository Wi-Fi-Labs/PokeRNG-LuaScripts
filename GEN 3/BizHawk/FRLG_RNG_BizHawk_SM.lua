...
function getBikeMod(rate)
 local isPlayerOnBike = (read8Bit(playerWalkRunStateAddr) & 6) ~= 0
...
function getBonusBall(speciesDexNumber, isSafariZone)
 ...
 elseif isBallSelected then
  ...
  elseif ballIndex == 10 then  -- Repeat ball catch rate
   ...
   local dexSeen1Flag = (read8Bit(saveBlock1Addr + 0x5F8 + dexIndex) & dexMask)
   local dexSeen2Flag = (read8Bit(saveBlock1Addr + 0x3A18 + dexIndex) & dexMask)
