...
local delay3 = 215
local delay6 = 2912
local initSeedBotState1 = savestate.create()
local initSeedBotState2 = savestate.create()
local initSeedBotState3 = savestate.create()
...
function getInstructionsInput()
 ...
 elseif mode[index] == "Initial Seed Bot" and showInstructionsText then
  ...
  gui.text(1, 28, "3) Advance a single frame (CTRL+N) while holding SELECT")
...
function showMissingStepsForEncounter()
 ...
 gui.text(1, 122, "Steps for wild encounter: "..encounterMissingSteps)
end
...
function advanceToDelay(delay, state)
 ...
 if state == 1 then
  savestate.save(initSeedBotState1)
 elseif state == 2 then
  savestate.save(initSeedBotState2)
 end
 ...
 while delay1 ~= delay5 and not initSeedFound do
  while delay2 ~= delay4 and not initSeedFound do
   while delay3 ~= delay6 and not initSeedFound do
    savestate.save(initSeedBotState3)
    joypad.set(1, {A = true})
    ...
    if not initSeedFound then
     delay3 = delay3 + 1
     initSeedWritten = false
     savestate.load(initSeedBotState3)
     emu.frameadvance()
    else
     break
    end
   ...
   if not initSeedFound then
    savestate.load(initSeedBotState2)
    delay2 = delay2 + 1
    delay3 = delay2 + 2
    delay4 = delay2 + 262
    delay6 = delay3 + 2697
  ...
  if not initSeedFound then
   savestate.load(initSeedBotState1)
   delay1 = delay1 + 1
   delay2 = delay1 + 5
   delay3 = delay2 + 2
   delay4 = delay2 + 262
