...
function calcAdvancesJump(seed)
 ...
 if tempCurr ~= current then
  ...
  while tempCurr ~= current and tempCurr2 ~= current and calibrationAdvances < 99999 do
...
function showRngInfo(initial, current, battle, advances)
 gui.text(0, 120, string.format("Initial Seed: %04X", initial))
 gui.text(0, 128, string.format("Painting Timer: %08X", read32Bit(timerAddr)))
 gui.text(1, 136, string.format("Battle Video Seed: %04X", battle))
 gui.text(1, 144, string.format("Current Seed: %08X", current))
