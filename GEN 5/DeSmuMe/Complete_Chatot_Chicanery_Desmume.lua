...
function main()
	...
	if key["9"] or key["numpad9"] then
		flipbtn = -1*flipbtn
	end
	
	if runbtn == 0 then
		gui.text(1,flipbtn*165,"Press 0 to run, advances set: "..maxAdvances,"cyan")
		gui.text(1,flipbtn*155,"Press 9 to switch screens","cyan")
	elseif runbtn == 1 then
		if pidAdvances <= maxAdvances then
			...
			gui.text(1, flipbtn*165, "Advances: " .. pidAdvances-1, "cyan")
		else
			gui.text(1, flipbtn*165, "Advances: " .. pidAdvances-1 .. ", DONE!","cyan")
		end
	end
	gui.text(1,flipbtn*175,"Game: "..game,"cyan")
end
